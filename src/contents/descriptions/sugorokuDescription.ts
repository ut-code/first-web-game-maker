...
level1~3 で段階的に作り方を学んでいきましょう。もちろん開発に自信がある方は飛び級しても大丈夫です。
...
すごろくの仕組みを変更してみましょう。
すごろくのマス目の長さを変更したい場合は、\`const BOARD_SIZE = 5\`の数字を書き換えることで変更できます。
...
Table を挿入するときは\`<head>\`と\`</head>\`の間に挿入しましょう。ここでは 4 人でプレイできるように必要な情報を表示する 4x3 枠の表を作成しています。各列にはプレイヤー名の入力欄、サイコロのボタン、出目が表示されます。ボタンは押したときのイベント処理として、後につくる”getNum()”を呼び出すようにしてあります。完成すると以下のように表示されます。
...
- 現在は 4 人のプレイヤーがプレイする前提ですごろくが作られていますが、1~4 人のどの人数でもプレイできるようにしてみましょう。
